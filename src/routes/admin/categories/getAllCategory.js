...
router.get('/get-all-category', upload.none(), (req, res) => {

  let page = parseInt(req.query.page);
  let size = parseInt(req.query.size);

  // Adding Foreign Key Association with vt_int_users to get the name of the user
  const creator = vt_categories.belongsTo(vt_int_users, {foreignKey: 'created_by'});
      ...
      vt_categories.findAndCountAll({
        ...
        .then((data) => {
          ...
          if(data !== null) {
            ...
            let pagination = {
              nextPageUrl: remaining === 0 ? '' : `${config.APP_URL}/get-all-category?page=${page + 1}&size=${size}`,
              prevPageUrl: page === 1 ? '' : `${config.APP_URL}/get-all-category?page=${page - 1}&size=${size}`,
